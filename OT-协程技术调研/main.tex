  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{enumerate}
        ...
        \item \textbf{CPU 密集}：1000 个并发任务，斐波那契(n=25)
        \item \textbf{上下文切换}：yield / channel / sleep(0) 量级测试
      \end{enumerate}
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{对比对象}
...
\begin{frame}{实验口径与局限}
  ...
  \begin{itemize}
    ...
    \item \textbf{内存测试}：Python 为 benchmark 期间峰值工作集；跨语言峰值 RSS 未统一采样，最终结论以耗时和机制解释为主。
  \end{itemize}
\end{frame}
