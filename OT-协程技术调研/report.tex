...
\begin{itemize}
  ...
  \item 图表生成：\verb|micromamba activate test| 后执行 \verb|python plot_results.py|。
\end{itemize}

测试分为三类：I/O 密集型测试创建 10000 个任务，每个任务 sleep 100 ms，用于模拟大量网络等待；CPU 密集型测试创建 1000 个任务，每个任务计算 \verb|fib(25)|；上下文切换测试使用大量 yield/sleep(0) 或轻量任务估计调度开销。完整代码见同目录下 \verb|bench.py|、\verb|bench.go| 和 \verb|Bench.java|。
...
\begin{table}[H]
  ...
  \begin{tabular}{lcc}
    \toprule
    模型 & 10000 任务耗时 & 相对线程基线 \\
    \midrule
    Python threading & 3.435 s & 1.0$\times$ \\
    Python asyncio & 0.464 s & 7.4$\times$ \\
    Go goroutine & 0.166 s & 20.7$\times$ \\
    Java platform thread（200 固定线程池） & 5.467 s & 1.0$\times$ \\
    Java virtual thread & 0.191 s & 28.6$\times$ \\
    \bottomrule
  \end{tabular}
\end{table}

结果符合预期：I/O 密集场景中，协程/虚拟线程能在等待期间释放执行权，使少量内核线程服务大量逻辑任务。Python asyncio 相比 Python threading 快约 7.4 倍；Go goroutine 和 Java 虚拟线程都能在 0.2 秒量级完成 10000 个 sleep 任务，明显优于基于平台线程的对照。
...
Python 的 threading 与 asyncio 在 CPU 密集型测试中耗时几乎相同，说明协程并不会减少计算本身的 CPU 成本。Go 和 Java 的 CPU 测试更快，主要原因不是 ``协程让计算变快''，而是运行时可以把任务分配到多核上执行。Java 平台线程池在该测试中略快于虚拟线程，也说明虚拟线程的优势不在短小 CPU 任务，而在大量阻塞等待任务。

切换开销图使用对数坐标展示量级差异。Python asyncio 的百万次 yield 约为 2.178 秒，Go goroutine 的 100000 次 channel 交互约为 145.7 ms，Java 虚拟线程的 100000 个 sleep(0) 任务约为 148 ms。由于不同语言的微基准并不完全等价，这些数字更适合用于判断量级，而不应作为绝对排名。
...
\begin{figure}[H]
  ...
  \caption{Python 进程 RSS 记录值对比}
  \label{fig:memory}
\end{figure}

本次 Python 脚本记录的是任务结束附近的进程 RSS，而不是统一采样得到的跨语言峰值 RSS，因此只用于辅助说明 asyncio 的对象与栈开销较低，报告不把它作为严格内存排名依据。若要进一步精确比较，应使用外部采样器或平台一致的监控工具记录峰值 RSS，并在 Java/Go 中补充 GC 前后内存统计。
