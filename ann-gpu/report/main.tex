...
本次 GPU 编程作业包括 HIP 云平台学习和 ANN GPU 加速实现两部分。HIP 部分在 AMD AUP 在线平台完成 Lab1--Lab6 的 notebook 填空与运行验证，内容覆盖向量加法、矩阵转置、直方图、归约、蒙特卡罗积分和 K-Means。在线学习的通过截图放在附录中，正文保留代码补全和分析填空答案，并总结各实验对应的 GPU 编程模式：合并访存、共享内存 tile、两级归约、减少全局原子操作、grid-stride 循环、多 kernel 编排以及 warp/wave 级归约。

ANN 部分面向近似最近邻搜索的 GPU 实现。针对单个 query 难以充分利用 GPU 的问题，本实验采用 batch 查询，将内积距离计算转化为矩阵乘法：
\[
...
实验工作分为 HIP 学习记录与 ANN GPU 实现两条线索。HIP 部分完成 AMD AUP 云端课程 Lab1--Lab6 的学习与验证，内容涉及向量加法、矩阵转置、直方图、归约、蒙特卡罗积分和 K-Means 等典型 GPU 程序。ANN 部分延续前序近似最近邻搜索实验，将原先主要运行在 CPU 上的距离计算和 Top-$k$ 后处理迁移到 GPU，并围绕 batch 矩阵乘法、Top-$k$ 合并和 IVF 分组搜索进行对照分析。

报告首先整理 HIP 学习中的主要代码补全、分析结果和通过截图，随后重点讨论 ANN GPU 实现。ANN 部分按问题建模、CUDA 程序设计、正确性与性能评估、实验分析和总结展开；HIP 学习中关于访存合并、共享内存、归约和多 kernel 编排的经验，也作为后续 ANN kernel 设计的参考。
...
AMD AUP HIP 学习部分共包含六个 notebook。每个 notebook 以一个典型 GPU 编程问题为主线，通过代码补全、运行结果观察和性能问题分析逐步展开。本节概述六个 Lab 的主题，并整理其中与索引计算、kernel 配置、访存模式和并行归约相关的主要答案。
...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \caption{CUDA batch ANN 搜索的数据流与阶段划分}
  \label{fig:cuda_flow}
\end{figure}
...
HIP 云学习部分系统梳理了 GPU 编程的共性方法：访存合并决定带宽型 kernel 的上限，共享内存适合做片上复用和重排，两级归约可以显著减少全局原子，warp/wave 内原语能够降低同步开销，多 kernel 编排则适合 K-Means 这类阶段性算法。

ANN GPU 实现部分完成了多条 GPU 搜索与对照路径。Flat 路径严格对应作业要求中的 batch 矩阵乘法，并进一步比较了手写 tiled GEMM、cuBLAS SGEMM、基础 Top-$k$ 和树形 Top-$k$；IVF 路径在 per-query probe list 基线之外，实现了按 probe 簇分桶的 grouped-IVF，对同一簇内向量矩阵与 query 小 batch 做 GEMM 式扫描。实测结果表明，\code{cublas_tree} 在 Recall@10 接近 1 时达到 83.24 $\mu$s/query；grouped-IVF 在 \code{nprobe=4} 时以 Recall@10 = 0.96280 换取 145.32 $\mu$s/query，在 \code{nprobe=8} 时以 189.90 $\mu$s/query 达到 Recall@10 = 0.99345。CUDA event、重复测量、cuobjdump 资源占用与 PTX/SASS 片段共同表明：ANN GPU 优化除距离计算外，还受到 Top-$k$ 合并和 IVF 候选组织方式的制约。
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{3.1cm}Y Y}
    ...
    报告保留 HIP 填空与学习截图，ANN GPU 加速部分按实验研究报告结构组织。 \\
    \midrule
    ANN GPU 路线 &
