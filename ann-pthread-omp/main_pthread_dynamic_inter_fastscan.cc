...
int main(int argc, char* argv[]) {
    ...
    pth_dyn::parallel_for(test_number, threads, [&](size_t i) {
        ...
        auto res = ann_fs::fastscan_search(fs_idx, base, test_query + i * vecdim, k, p);
    ...
    CsvWriter csv;
    csv.append({"fastscan", "pthread", "inter", "dynamic", threads, "static", 0,
                p, static_cast<int>(base_number), avg_recall, avg_latency, total_wall});
