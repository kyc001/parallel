...
#include "pthread_intra_barrier.h"
...
int main(int argc, char* argv[]) {
    ...
    pth_bar_intra::Pool pool;
    pth_bar_intra::init(pool, threads);
    ...
    for (size_t qi = 0; qi < test_number; ++qi) {
        ...
        pth_bar_intra::run(pool, [&](int tid) {
            ...
            if (lo < hi) {
                locals[static_cast<size_t>(tid)] =
                    pq_intra::chunk_coarse_lut(pq_index, lut.data(), lo, hi, rerank_p);
            }
        });
    ...
    CsvWriter csv;
    csv.append({"pq_simd", "pthread", "intra", "barrier", threads, "static", 0,
