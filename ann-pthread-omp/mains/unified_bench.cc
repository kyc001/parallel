...
static void RunPQ(const DataCtx& ctx) {
    Section("PQ");
    PQIndex pq_idx; pq_idx.build(ctx.base.get(), ctx.base_n, ctx.d, 8, 256, 8);
    for (int nthr : {1, 2, 4, 8, 16}) {
        { std::vector<FlatHeap> res;
          double us = TimeOnce_us([&]{ pq_search_inter_omp(ctx.base.get(),ctx.queries.get(),ctx.base_n,ctx.query_n,ctx.d,kK,pq_idx,100,nthr,res); });
          Print("pq","omp_inter",nthr, RecallAtK_Float(res,ctx.gt.get(),ctx.query_n,ctx.gt_dim), us/ctx.query_n, "p=100"); }
        { std::vector<FlatHeap> res;
          double us = TimeOnce_us([&]{ pq_search_inter_static(ctx.base.get(),ctx.queries.get(),ctx.base_n,ctx.query_n,ctx.d,kK,pq_idx,100,nthr,res); });
          Print("pq","pthread_static_inter",nthr, RecallAtK_Float(res,ctx.gt.get(),ctx.query_n,ctx.gt_dim), us/ctx.query_n, "p=100"); }
        { std::vector<FlatHeap> res;
          double us = TimeOnce_us([&]{ pq_search_inter_dynamic(ctx.base.get(),ctx.queries.get(),ctx.base_n,ctx.query_n,ctx.d,kK,pq_idx,100,nthr,res); });
          Print("pq","pthread_dynamic_inter",nthr, RecallAtK_Float(res,ctx.gt.get(),ctx.query_n,ctx.gt_dim), us/ctx.query_n, "p=100"); }
        { std::vector<FlatHeap> res;
