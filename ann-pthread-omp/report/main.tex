...
\textbf{Pthread dynamic inter-query 的关键并行结构}：以 Flat 为例，每个 worker 通过 \code{std::atomic::fetch\_add} 主动抢下一条 query，不需要预先切分任务，自然吸收 query 间耗时差异：
...
\textbf{Thread pool 的 mutex + condition variable 同步路径}：pool 预创建 $T$ 个 worker 线程，通过 mutex 保护共享任务队列、cv 唤醒空闲 worker，避免频繁 \code{pthread\_create}/\code{pthread\_join} 的开销：
...
与 dynamic 的 \code{fetch\_add} 相比，pool 的 mutex+cv 在粒度较细时（如 IVF 的 \code{nprobe=4}）能复用线程避免空转，但锁的 acquire/release 成本（约 20--50 cycle）在大粒度任务中相对可忽略。这就是表 \ref{tab:best} 中 IVF/IVF-PQ 的 pool 略优的微观原因。
...
时间度量使用 \code{std::chrono::high\_resolution\_clock}，在被测函数前后各取一次时间戳，差值即为端到端延迟。每个数据点只跑一次（2000 条 query 的批量检索），不取多次平均——原因是本实验关注的是\textbf{稳态延迟}而非吞吐量，且 Windows 上后台进程的干扰使多次运行的方差较大，单次测量的可重复性反而更好（相同配置多次跑的差异 $<5\%$）。
...
§\ref{sec:scheduler} 给出了 OMP\_inter / Pthread static / dynamic / pool 四种调度的对比，但只能覆盖 OpenMP 默认 schedule(static)。本节用 \code{\#pragma omp parallel for schedule(runtime)} 配合 \code{OMP\_SCHEDULE} 环境变量做 12 组配置的扫描，让 schedule 策略与 chunk 大小成为运行时参数，结果见表 \ref{tab:omp_sweep}（Flat inter-query, 16 线程）。

\begin{table}[H]
  \centering\small
  \begin{tabular}{lrrr}
    \toprule
    schedule & chunk & latency / $\mu$s & 相对 guided,8 \\
    \midrule
...
PQ 的 $p$ 控制粗排后进入精排的候选数。表 \ref{tab:pq_p_sweep} 给出完整的 $p$-sweep 数据（\code{pthread\_dynamic\_inter}, t=16）。$p=100$ 时 Recall@10 仅 0.70，$p=500$ 时过 0.94 门槛，$p=1000$ 达到 0.98 是本文默认高召回点，$p=2000$ 进一步到 0.996 但延迟翻倍，$p=5000$ 时 Recall 接近 1.0 但延迟已达 1121 μs——收益快速衰减。
...
\begin{table}[H]
  ...
  \begin{tabular}{rrr}
    ...
    \bottomrule
  \end{tabular}
  \caption{FastScan $p$-sweep 的 recall-latency trade-off（\code{pthread\_dynamic\_inter}, t=16）}
  \label{tab:fastscan_p_sweep}
\end{table}

表 \ref{tab:ivf_nlist_sweep} 给出 IVF 的 \code{nlist}-sweep（固定 \code{nprobe=nlist/4}，\code{pthread\_pool\_inter}, t=16）。\code{nlist=16}（核心矩阵默认）在 0.96 recall 下延迟最低（115 μs）；增大 \code{nlist} 到 64/128/256 能把 recall 抬到 0.99+，但延迟增幅有限（119--141 μs），说明 IVF 的粗排阶段（$O(nlist \cdot d)$）在 \code{nlist} 较小时不是瓶颈，真正的开销在 inverted list 扫描。
...
{\small $^\dagger$ \code{hnsw\_baseline} 的 65.48\,$\mu$s 与 ef sweep 中 ef\,=\,50 的 61.92\,$\mu$s 存在微小差异，原因是两次测量分属不同 run，系统噪声和 cache 状态略有不同；两者的 Recall@10 一致（0.96865）。}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{5.8cm}rp{2.5cm}Y}
    ...
    \midrule
    \code{argmin\_l2\_blocked} & 15.49 & pq\_blocked\_avx2.h & PQ 编码 / 最近 centroid 搜索，是离线阶段最重的 SIMD FMA 循环。\\
    \code{\_mm256\_fmadd\_ps} & 4.16 & fmaintrin.h & 证明热点中大量时间花在 AVX2 FMA。\\
    \code{adc\_distance} & 2.37 & pq\_scan\_avx2.h & PQ 查询阶段 ADC 查表累加。\\
    \code{ann\_avx2::ip\_distance\_avx2} & 2.21 & flat\_scan\_avx2.h & Flat / rerank 的 256-bit 内积距离。\\
    \code{\_unguarded\_partition} & 1.57 & stl\_algo.h & 候选 partial sort / nth-element 相关开销。\\
    \code{train\_fastscan} & 0.77 & pq\_fastscan\_avx2\_safe.h & FastScan 训练与码本准备。\\
    \code{pq\_search} & 0.70 & pq\_scan\_avx2.h & 端到端 PQ 查询封装。\\
    \bottomrule
  \end{tabularx}
...
\textbf{片段 1：PQ / Flat 的 AVX2 FMA 内积循环}（\code{pq\_search} 热点）。每次迭代用 \code{vmovups} 加载 8 个 float（256-bit），再用 \code{vfmadd231ps} 完成乘累加，最后 \code{vhaddps} + \code{vextractf128} 做水平归约：
...
4 路 FMA 流水线使单次迭代处理 32 个 float（4 组 $\times$ 8 路 SIMD），对 $d=96$ 只需 3 次迭代即可完成一条 base 的内积。这与 VTune 中 \code{argmin\_l2\_blocked}（15.49 s）和 \code{\_mm256\_fmadd\_ps}（4.16 s）占主导的现象一致。

\textbf{片段 2：Pthread dynamic worker 的原子抢任务}（\code{InterDynamicWorker}）。用 \code{lock xadd} 实现 \code{std::atomic::fetch\_add}，一次原子操作即完成"读当前 query id + 自增"：
...
SSE 路径单次处理 4 float，比 AVX2 的 8 float 慢约 $2\times$，但 HNSW 的主要瓶颈不在距离计算——图遍历的 \code{priority\_queue} 操作、visited 标记检查和邻居扩展的 scalar branch 才是控制流热点。这与 VTune 中 HNSW 热点分布（distance 函数占比低于 Flat/PQ）和表 \ref{tab:hnsw} 中 HNSW baseline 65 $\mu$s 已很快但多线程难以加速的现象一致。
...
该方案正确，但我补充了一个细节：\code{schedule(static)} 不带 chunk 时，OMP runtime 会自动按 \code{query\_n / nthreads} 等分，这与显式指定 \code{chunk=125}（2000/16）的行为不完全相同——runtime 的内部实现可能做 cache line 对齐优化。因此我在 sweep 中保留了``default static''作为对照点，实测 318 μs 确实优于所有显式 chunk 变体，验证了这一猜测。12 组数据纳入报告 §\ref{sec:omp_sweep}。
...
本实验在代码结构梳理、脚本编排、图表生成、LaTeX 排版和一致性检查中使用了 OpenAI Codex 等大语言模型辅助；所有算法实现、参数选择、实验运行、性能数据采集和主要结论均由本人完成，AI 不用于生成或伪造实验数据。所有正文数字均可由实验输出文件复核。

\nocite{babenko2016efficient,jegou2011product,andre2015fastscan,malkov2020hnsw,johnson2021faiss,mattson2004patterns,openmp2018}
