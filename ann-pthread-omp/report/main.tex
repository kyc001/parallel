...
本实验覆盖了指导书 6 项进阶要求中的 4 项：多平台 x86/ARM 对比（§6.11）、C++ \code{std::thread} 与 Pthread/OpenMP 三方对比（如下）、生成式 AI 辅助（附录 A）、以及其他优化策略（虚假共享、P/E 亲和性、PQ 编码多线程等）。OpenMP GPU 卸载（\code{\#pragma omp target}）和 oneAPI/SYCL 对比未涉及，原因是本机 MSYS2/GCC 不支持 OpenMP target offload（需 Intel oneAPI DPC++ 编译器），且 SYCL 需要额外的编译器和运行时环境。

\textbf{C++ \code{std::thread} vs Pthread vs OpenMP 三方对比}（Flat inter-query, 鲲鹏 920）：
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrrr}
    ...
    方法 & T=1 & T=2 & T=4 & T=8 \\
    \midrule
    std::thread dynamic & 11983 & 3712 & 2183 & 1131 \\
    Pthread dynamic     & 10870 & 4026 & 1941 & 1115 \\
    OpenMP static       &  9414 & 6006 & 3639 & 1625 \\
    \bottomrule
  \end{tabular}
  \caption{Flat inter-query 三方对比延迟（μs/query, 鲲鹏 920, Recall@10=0.99995）}
  \label{tab:stdthread}
\end{table}

\code{std::thread} 与 Pthread 在多线程下性能接近（t=8: 1131 vs 1115 μs），因为两者底层都使用 \code{std::atomic::fetch\_add} 实现动态任务分配，POSIX 线程的额外开销在每条 query 百 μs 级的粒度下可忽略。OpenMP 在 t=8 时比前两者慢约 46\%（1625 vs 1115 μs），原因是 \code{schedule(static)} 的固定划分在鲲鹏 8 核上无法吸收 query 间耗时差异——尽管 Flat 的单 query 工作量理论上均匀，但 NUMA 和 cache 状态差异仍导致尾部等待。这与 §6.5 中"OpenMP \code{dynamic,8} 接近 Pthread"的结论一致——若 OpenMP 也用 \code{schedule(dynamic,1)} 则差距会缩小。
