...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{1.8cm}p{2.6cm}p{2.6cm}Y}
    ...
    \bottomrule
  \end{tabularx}
  \caption{Flat inter-query 的 Pthread vs OpenMP 编程对比}
...
为了量化 strong scaling 在不同问题规模下的表现，本节固定算法（Flat inter-query, OpenMP \code{schedule(static)}, AVX2 内积内核）做 N $\in \{10k, 50k, 100k\}$ $\times$ T $\in \{1, 2, 4, 8, 16\}$ 的二维扫描。
...
\begin{table}[H]
  ...
  \begin{tabular}{rrrrrrr}
    ...
    \midrule
    2 & 0.97 & 0.41 & $2.4\times$ & 0.13 & 0.00 & $42.4\times$ \\
    4 & 0.55 & 0.14 & $4.0\times$ & 0.15 & 0.00 & $55.8\times$ \\
...
本实验覆盖了指导书 6 项进阶要求中的 5 项：多平台 x86/ARM 对比（§6.11）、C++ \code{std::thread} 与 Pthread/OpenMP 三方对比（如下）、生成式 AI 辅助（附录 A）、其他优化策略（虚假共享、P/E 亲和性、PQ 编码多线程等）、以及编程工具对比分析（如下）。
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrrr|rrrr}
    \toprule
    & \multicolumn{4}{c|}{i9-13900H (本机)} & \multicolumn{4}{c}{鲲鹏 920} \\
    方法 & T=1 & T=4 & T=8 & T=16 & T=1 & T=4 & T=8 \\
    \midrule
    std::thread & 6138 & 588 & 391 & 316 & 11983 & 2183 & 1131 \\
    Pthread     & 5370 & 596 & 337 & 254 & 10870 & 1941 & 1115 \\
    OpenMP      & 3774 & 595 & 343 & 317 &  9414 & 3639 & 1625 \\
    \bottomrule
  \end{tabular}
  \caption{Flat inter-query 三方对比延迟（$\mu$s/query）}
  \label{tab:stdthread}
\end{table}
...
尽管无法实际编译运行，我们编写了 SYCL 和 OpenMP offload 版本的 Flat scan 代码（见附录 B），展示了对 GPU offload 编程模型的理解。核心差异在于：GPU offload 方案需要将 base 数据（100K$\times$128$\times$4B $\approx$ 50 MB）通过 PCIe 传输到设备内存，单次传输延迟约 3 ms（PCIe 4.0 $\times$16, 16 GB/s），远超 CPU 搜索延迟（50--130 μs）。因此即使工具链可用，GPU offload 在当前问题规模（100K 向量、128 维）下也不具备性能优势——这与 CPU-GPU 协同计算的经典结论一致：GPU offload 适合计算密集型任务（如大规模矩阵乘法），而非内存密集型的 ANN 搜索。
...
\begin{itemize}
  \item \textbf{PCIe 传输开销}：base 数据 100K$\times$128$\times$4B $\approx$ 50 MB，PCIe 4.0 x16 带宽 16 GB/s，单次传输 $\sim$3 ms。
  \item \textbf{CPU 搜索延迟}：IVF-PQ 134 $\mu$s/query，HNSW 65 $\mu$s/query。
  \item \textbf{结论}：PCIe 传输延迟（3 ms）$\gg$ CPU 搜索延迟（0.065--0.134 ms），GPU offload 在内存密集型 ANN 搜索中不可行。GPU 适合计算密集型任务（如大规模矩阵乘法），而非带宽受限的近邻搜索。
\end{itemize}
