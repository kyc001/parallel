...
PQ 的 $p$ 控制粗排后进入精排的候选数。$p=100$ 时延迟仅 85.43 $\mu$s，但 Recall@10 只有 0.70780；$p=1000$ 时 Recall@10 提升到 0.98335，延迟为 136.47 $\mu$s，是本文的默认高召回点；$p=2000$ 进一步达到 0.99560，但延迟升至 183.35 $\mu$s。

IVF 的 \code{nprobe} 控制扫描簇数。\code{nprobe=1} 只有 0.73665 recall，\code{nprobe=4} 达到 0.96225，\code{nprobe=16} 接近精确但延迟明显上升。IVF-PQ 的 local 构建在固定 $p=100$ 时 recall 始终高于 global，证明局部码本降低了簇内量化误差；但两者在该 $p$ 下都未过 0.95 门槛，因此主线高召回结果把 local 的 $p$ 提到 1000。
