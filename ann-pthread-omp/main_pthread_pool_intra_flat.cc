...
int main(int argc, char* argv[]) {
    ...
    test_number = 2000;
    const size_t k = 10, chunk_size = 1024;
    ...
    for (size_t qi = 0; qi < test_number; ++qi) {
        ...
        pth_pool_intra::run(pool, base_number, chunk_size,
            ...
            auto partial = flat_intra::chunk_topk(
                base, test_query + qi * vecdim, lo, hi, vecdim, k);
            flat_intra::merge_into(locals[static_cast<size_t>(tid)],
                                   std::move(partial), k);
        });
    ...
    CsvWriter csv;
    csv.append({"flat_simd", "pthread", "intra", "pool", threads, "dynamic",
                static_cast<int>(chunk_size), 0, static_cast<int>(base_number),
