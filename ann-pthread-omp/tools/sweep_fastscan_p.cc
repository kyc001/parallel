// FastScan p-sweep: fixed pthread_dynamic_inter, t=16, sweep rerank_p
// 编译: g++ tools/sweep_fastscan_p.cc -o build/sweep_fastscan_p.exe -O2 -mavx2 -mfma -fopenmp -lpthread -std=c++11 -I.
...
#include "simd/pq_fastscan_avx2_safe.h"
...
int main(int argc, char** argv) {
    ...
    train_fastscan(idx, base.get(), static_cast<int>(base_n), static_cast<int>(base_d));
    encode_fastscan(idx, base.get());

    std::vector<ann_fastscan_pthread::SearchHeap> results;
    const auto t1 = std::chrono::high_resolution_clock::now();
    fastscan_search_inter_dynamic(idx, base.get(), queries.get(), query_n, k, p, nthreads, results);
    ...
    std::cout << "fastscan,p=" << p << ",recall=" << total_recall / query_n << ",latency_us=" << us << "\n";
    return 0;
}
