...
int main(int argc, char* argv[]) {
    ...
    test_number = 2000;
    const size_t k = 10, rerank_p = 100, chunk_size = 1024;
    ...
    for (size_t qi = 0; qi < test_number; ++qi) {
        ...
        pth_pool_intra::run(pool, base_number, chunk_size,
            ...
            auto partial = sq_intra::chunk_coarse(
                sq_index, query_code.data(), vecdim, lo, hi, rerank_p);
            sq_intra::merge_into(locals[static_cast<size_t>(tid)],
    ...
    CsvWriter csv;
    csv.append({"sq_simd", "pthread", "intra", "pool", threads, "dynamic",
