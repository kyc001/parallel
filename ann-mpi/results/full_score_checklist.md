...
|---|---|---|
| Use `mpic++`, not the old `test.sh` submission path | `Makefile:1` sets `CXX = mpic++`; `results/kunpeng_smoke.txt:15` records `/usr/local/bin/mpic++ main.cc -o main ...` | Done |
...
| Support two platforms | Windows log `results/local_cross_platform.txt`, Kunpeng direct log `results/kunpeng_smoke.txt`, PBS log `results/kunpeng_pbs_smoke.txt`, summary table `results/cross_platform_summary.txt` | Done |
| PBS run is recorded | `results/kunpeng_pbs_smoke.txt:13-14` records the exact `qsub -v ...` command and job `26866.master_ubss1` | Done |
...
| Process/thread count is visible | Logs print `mpi_procs=2` and `nthreads=2`; scripts expose `NP` and `OMP_NUM_THREADS` in `qsub_mpi.sh:10-11`, `scripts/run_kunpeng_smoke.sh:7-8`, and `scripts/run_local_cross_platform.ps1:3-4` | Evidence ready |
...
| Recall-latency trade-off | Summary compares IVF-PQ, block-HNSW, IVF+HNSW, and HNSW-on-HNSW under `NP=2`, `threads=2`, `query_n=200`; `HNSW_ON_HNSW_NPROBE=16` is recorded in all logs | Done |

## Graph-Index Requirement

| Requirement | Evidence | Status |
|---|---|---|
| Option A: IVF + HNSW | Mode `ivf-hnsw` is parsed at `main.cc:128-132`; local nested indexes are built at `main.cc:436-437`; implementation lives in `hnsw/hnsw_ivf_nested.h:13-79`; logs show `ivf_hnsw_nested_mpi_omp` Recall@10 `0.95400` on Windows and Kunpeng | Done |
...
| Option C: other combination strategy | Mode `hnsw-on-hnsw` is parsed at `main.cc:135-138`; implementation `hnsw/hnsw_on_hnsw.h:24-117` builds a top HNSW over block centroids plus one HNSW per block; logs show Recall@10 `1.00000` on Windows and Kunpeng | Done |
...
|---|---|---:|---:|---:|
| Windows local MS-MPI | IVF-PQ | 0.95650 | 229.98250 | 1.38700 |
| Windows local MS-MPI | block-HNSW | 1.00000 | 269.52450 | 2.80250 |
| Windows local MS-MPI | IVF+HNSW nested | 0.95400 | 154.56800 | 1.99750 |
| Windows local MS-MPI | HNSW-on-HNSW | 1.00000 | 461.18750 | 1.76650 |
| Kunpeng direct mpiexec | IVF-PQ | 0.95650 | 360.83221 | 1.91569 |
| Kunpeng direct mpiexec | block-HNSW | 1.00000 | 277.39286 | 1.72496 |
| Kunpeng direct mpiexec | IVF+HNSW nested | 0.95400 | 567.97028 | 2.36988 |
| Kunpeng direct mpiexec | HNSW-on-HNSW | 1.00000 | 1851.95684 | 2.46167 |
| Kunpeng PBS qsub | IVF-PQ | 0.95650 | 438.50303 | 1.69635 |
| Kunpeng PBS qsub | block-HNSW | 1.00000 | 260.82397 | 1.58548 |
| Kunpeng PBS qsub | IVF+HNSW nested | 0.95400 | 512.36391 | 1.81317 |
| Kunpeng PBS qsub | HNSW-on-HNSW | 1.00000 | 1741.80984 | 1.75953 |

## Report Pointers

- IVF-PQ and IVF+HNSW nested trade slightly lower recall for lower or different
  online costs depending on platform.
- Block-HNSW and HNSW-on-HNSW both reach Recall@10 `1.00000` in this smoke
  setting. HNSW-on-HNSW is much slower on Kunpeng because `nprobe_blocks=16`
...
- `comm+merge latency` is consistently small compared with `max local search
  latency`, so this `NP=2` experiment is dominated by local search work.
