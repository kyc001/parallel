...
run_step "MPI IVF+HNSW nested smoke" "$MPIEXEC" -np "$NP" ./main "$THREADS" 16 4 50 "$QUERY_N" ivf-hnsw
run_step "MPI HNSW-on-HNSW smoke" "$MPIEXEC" -np "$NP" ./main "$THREADS" 16 "$HNSW_ON_HNSW_NPROBE" 50 "$QUERY_N" hnsw-on-hnsw
