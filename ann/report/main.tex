...
本实验在代码调试、脚本拼接、\LaTeX{} 排版和报告一致性复核过程中使用过在线大语言模型（豆包、OpenAI Codex）做辅助查询，主要用于 AVX2/NEON intrinsic 参数校对、\LaTeX{} 格式修正、PQ/FastScan 优化路线的技术验证，以及表述与实测记录的一致性检查。所有算法设计、参数选择、实验方案、数据采集、性能数据来源与主要结果分析均由本人完成；AI 辅助不用于生成性能数据。以下是两个代表性咨询片段。
...
豆包确认了 nibble 拆分流程和 AVX2/NEON 语义差异，最关键的是累加上界提醒——我最初忽略了 16 段可能溢出 255。最终实现保留 uint8 饱和加法，但将 LUT 线性缩放到每段 $\le$ 15，使 $M=16$ 时总和上界为 240，从而不会实际触发饱和。表 \ref{tab:fastscan} 与表 \ref{tab:fastscan_kunpeng} 的端到端结果均为本人独立实测，nibble 拆分正确性通过与标量 FastScan 查表版本对照进行了验证。
...
> 3. NEON \texttt{vqtbl1q\_u8} 为 128-bit 宽度（16 路查表），AVX2 \texttt{pshufb} 为 256-bit 内两条独立 128-bit lane。AVX2 侧需额外处理 lane 交叉。

\paragraph{我的判断、修改与结果复核}

豆包关于 gather 吞吐的提醒与 Intel 手册一致，坚定了我放弃 gather 的路线。NEON 128-bit lane vs AVX2 256-bit lane 的差异是我在实现中需要注意的关键点。最终路线（$K_s=16$ + shuffle）由我独立决策并实现，表 \ref{tab:gather_vs_shuffle} 微基准（gather 慢标量 3.9$\times$）与表 \ref{tab:fastscan} 端到端结果均为本人独立采集复核。

\medskip
\noindent 综上，AI 辅助限于 intrinsic 语法核对、手册速查、跨平台差异提示和报告一致性复核；所有关键技术决策、实验设计、数据采集与主要数据分析均来源本人工作，未使用 AI 生成任何性能数据，图表也均由脚本根据实测数据绘制。
