...
需要澄清的是，表 \ref{tab:gather_vs_shuffle} 是单条 ADC 查表的微基准（ns/row），shuffle 与标量持平不意味着对端到端无贡献。FastScan 的加速来自三项联合收益：(1) 4-bit LUT 从 $M\times 256\times 4$ B 缩至 $M\times 16$ B，批量扫描时 LUT 可驻留 L1；(2) double-nibble 打包使 $M=16$ 仍保持 8 B/code；(3) shuffle 将 32 次标量查表合并为单条寄存器内操作，消除标量循环的分支与地址计算开销。VTune 显示标准 PQ 的 Retiring 占比 68.5\% 而 Memory Bound 仅 0.6\%，印证瓶颈在控制开销而非带宽——shuffle 正好针对此瓶颈。
...
\noindent\textbf{关于鲲鹏相对加速比系统性地高于 i9 的讨论。} 表 \ref{tab:core_compare} 中鲲鹏 920（128-bit NEON）在所有方法上的相对基线加速比均高于 i9（256-bit AVX2）：Flat-SIMD $2.77\times$ vs $2.60\times$，FastScan $12.03\times$ vs $8.44\times$。原因在于鲲鹏串行基线（$16120.74\,\mu\text{s}$）是 i9 的 $3.53$ 倍，远超频率和 SIMD 宽度的硬件差距——GCC 10.3.1 对标量代码的优化可能弱于 GCC 14.2.0，微架构与 cache 子系统的差异也会共同影响。较慢的基线放大了相对加速比，这是该指标作为跨平台比较的内在局限，因此本文始终同时报告绝对时延。鲲鹏缺少 perf 数据（见 2.3 节），但汇编输出显示其标量内循环以 \texttt{fmla} 单发射为主，未做有效的循环展开或指令重排，为上述推断提供了定性支撑。
...
本实验在代码调试、脚本拼接和 \LaTeX{} 排版过程中使用过在线大语言模型（豆包）做辅助查询，主要用于 AVX2/NEON intrinsic 参数校对、\LaTeX{} 格式修正及 PQ/FastScan 优化路线的技术验证。所有算法设计、参数选择、实验方案、数据采集、结果分析与报告组织均由本人独立完成。以下是两个代表性咨询片段。
...
豆包确认了 nibble 拆分流程和 AVX2/NEON 语义差异，最关键的是 16-bit 累加提醒——我最初忽略了 16 段可能溢出 255。我改用 \texttt{\_mm256\_sad\_epu8} 做无饱和水平求和，同时将 LUT 线性缩放到每段 $\le$ 15。表 \ref{tab:fastscan} 与表 \ref{tab:fastscan_kunpeng} 的端到端结果均为本人独立实测，nibble 拆分正确性通过对比 FastScan 粗排与标准 PQ ADC 粗排结果的一致性进行了验证。
...
\medskip
\noindent 综上，AI 辅助限于 intrinsic 语法核对、手册速查和跨平台差异提示；所有关键技术决策、代码实现、实验设计与数据分析均来源本人独立工作，未使用 AI 生成任何性能数据或图表。
