...
\begin{table}[H]
  ...
  \begin{tabular}{p{2.7cm}p{2.3cm}p{3.2cm}p{4.7cm}}
    ...
    鲲鹏 920 服务器 & AArch64, NEON asimd / asimddp (128-bit) & openEuler 22.03 (LTS-SP4), GCC/G++ 10.3.1，以 \texttt{bash test.sh 1 1} 提交运行 & ARM 正式实验数据采集 \\
...
\paragraph{软件预取。}
E2 在 Flat-SIMD 流式扫描上尝试多种预取距离。所有显式预取都没有超过无预取基线，说明该访问模式足够顺序，硬件预取器已经覆盖主要需求。
...
本实验在代码调试、脚本拼接和 \LaTeX{} 排版过程中使用过在线大语言模型（豆包）做辅助查询，主要用于 AVX2/NEON intrinsic 参数顺序校对、\LaTeX{} 表格与参考文献格式修正。所有算法设计、参数选择、实验方案、数据采集、结果分析与报告组织均由本人独立完成。以下按``问题 $\to$ AI 回复要点 $\to$ 我的判断与修改 $\to$ 最终实测结果''的结构给出一个具有代表性的咨询片段，以说明典型的使用方式。
...
在完成标准 PQ 实现之后，通过 VTune 观察到 ADC 粗排阶段存在明显的随机内存访问（$O(NM)$ 次 LUT 间接索引），属于 memory-bound 内核。直接想到的两种 SIMD 改造方向是：(a) 用 \texttt{\_mm256\_i32gather\_ps} 把 8 个 centroid 的 LUT 取值合并到一条指令；(b) 把 $K_s$ 从 256 压到 16，用 \texttt{\_mm256\_shuffle\_epi8} 做寄存器内查表（FastScan 思路）。根据 Intel 优化手册中``gather 吞吐随索引散度线性恶化''的说明，以及 AArch64 NEON 没有等价 gather 的事实，我判断路线 (b) 更可移植、也更契合两平台硬件特点。
 ...
 \paragraph{AI 回复要点（摘要）。}
 我向豆包求证的主要是实现细节：$K_s=16$、$M=16$ 下 code 以 double-nibble 方式打包成 1 字节后，\texttt{shuffle\_epi8} 的掩码构造是否会出现 nibble 拆分错位；\texttt{adds\_epu8} 多层累加下的饱和边界如何设置；以及跨平台头文件 dispatch 的常见写法。工具给出的回复可概括为三点：其一，建议把高/低 nibble 的掩码拆开验证，避免表项错位；其二，建议在每段 LUT 量化时显式记录缩放区间，避免 8-bit 饱和累加过早溢出；其三，建议把 AVX2 / NEON 的 intrinsic 包装在统一头文件下，通过编译期开关选择实现。

 \paragraph{我的判断与修改。}
 这些回复主要提供了 intrinsic 名、掩码常量与头文件组织方式的参考，真正决定路线的仍是我自己的实验判断：我没有采用 gather 路线，而是保留 ``$K_s=16$ + shuffle'' 的 FastScan 设计；同时把 LUT 量化比例系数做成每段独立 min/max，把 rerank 阈值改成编译期宏，规避远程作业环境变量丢失，并为 Windows / AArch64 分别补齐可编译的头文件 dispatch。

 \paragraph{最终设计与复核。}
 4.6 节 Gather vs Shuffle 微基准的实测（gather 比 scalar 慢 $3.9\times$、shuffle 与 scalar 持平）以及正式端到端结果都由本人独立完成并复核。最终 i9 与鲲鹏两平台上的 FastScan 相对标准 PQ 分别取得 $1.41\times$ / $1.48\times$ 提升（表 \ref{tab:fastscan}、\ref{tab:fastscan_kunpeng}），说明本人的路线判断与后续修改确实转化成了可复现的性能收益。
